...
!developmentChains.includes(network.name)
    ...
    : describe("Dynamic SVG NFT Unit Tests", function () {
          ...
          describe("svgToImageURI", () => {
              it("base64 encodes svg images", async () => {
                  ...
                  assert.equal(await dynamicSvgNft.svgToImageURI(lowSVG), lowSVGImageuri)
                  assert.equal(await dynamicSvgNft.svgToImageURI(highSVG), highSVGimageUri)
              })
          })
          ...
          describe("mintNft", () => {
              ...
              it("shifts the token uri to lower when the price does not surpass the highvalue", async () => {
                  const highValue = ethers.utils.parseEther("100000000") // $100,000,000 doller per ether
                  const txResponse = await dynamicSvgNft.mintNft(highValue)
                  await txResponse.wait(1)
                  const tokenURI = await dynamicSvgNft.tokenURI(0)
                  assert.equal(tokenURI, lowTokenUri)
              })
              it("map high value to token id", async () => {
                  const highValue = ethers.utils.parseEther("2")
                  const txResponse = await dynamicSvgNft.mintNft(highValue)
                  await txResponse.wait(1)
                  const result = await dynamicSvgNft.s_tokenIdToHighValue(0)
                  assert.equal(result.toString(), highValue.toString())
              })
          })

          describe("tokenURI", () => {
              it("reverts for nonexistent token", async () => {
                  await expect(dynamicSvgNft.tokenURI(1)).to.be.reverted
              })
          })
      })
